...
const Canteens = () => {
  ...
  const columns = [
    ...
    {
      name: t('posSessions:field.canteen'),
      cell: (row) => {
        return <Link to={`/admin${RouteBase.CanteenDetail}?id=${row?.id}`}>{row.name}</Link>;
      },
      ignoreRowClick: true,
      allowOverflow: true,
      sortable: true,
  ...
  const getCanteensList = useGetData(TRANSACTION_URL.GET_LIST_CANTEENS, initParams);
  const getSchoolList = useGetData(TRANSACTION_URL.GET_LIST_SCHOOL);

  useEffect(() => {
    let isCurrent = true;
    if (!!isCurrent) {
      void getCanteensList._getData(null, paramRequest);
    }
    return () => {
      isCurrent = false;
    };
  ...
  function onDelete(id) {
    httpServices.deleteData(`${TRANSACTION_URL.GET_LIST_CANTEENS}/${id}`, null).then(
      (_) => {
        onSearch();
      },
      (reason) => {
  ...
  function navigateToDetail(id) {
    history.push(`/admin${RouteBase.CanteenDetail}?id=${id}`);
  }
    ...
    <BaseAdminContainer>
      <Container className="mt-3" fluid>
              ...
              <Form>
                <div className="row form-group">
                  <div className="col-sm-3">
                    <Label>{t('posSessions:field.canteen')}</Label>
        ...
        <Card className="shadow mt-3">
          ...
          <CardBody>
            <CustomDataTable
              data={getCanteensList?.data?.data}
              columns={columns}
              progressPending={getCanteensList?.isLoading}
              pagination
              paginationServer
              paginationTotalRows={getCanteensList?.data?.pagination?.totalRecords ?? 0}
